...
{
#ifdef FC_USE_SDL_GPU
    if (rect != NULL) GPU_SetClipRect(dest, *rect);
    else GPU_UnsetClip(dest);
#elif defined(ENABLE_SDL_CLIPPING)
...
{
    if (ASCII_LATIN_1_STRING == NULL) ASCII_LATIN_1_STRING = new_concat(FC_GetStringASCII(), FC_GetStringLatin1());
...
FC_Rect
FC_DefaultRenderCallback(FC_Image* src, FC_Rect* srcrect, FC_Target* dest, float x, float y, float xscale, float yscale)
{
...
static FC_Rect (*fc_render_callback)(FC_Image*  src,
                                     FC_Rect*   srcrect,
                                     FC_Target* dest,
                                     float      x,
                                     float      y,
...
void FC_SetRenderCallback(
    FC_Rect (*callback)(FC_Image* src, FC_Rect* srcrect, FC_Target* dest, float x, float y, float xscale, float yscale))
{
    if (callback == NULL) fc_render_callback = &FC_DefaultRenderCallback;
    else fc_render_callback = callback;
}
...
Uint32 FC_GetCodepointFromUTF8(const char** c, Uint8 advance_pointer)
{
    ...
    str = *c;
    if ((unsigned char)*str <= 0x7F) result = *str;
...
void FC_SetBufferSize(unsigned int size)
{
    ...
    else fc_buffer = (char*)malloc(fc_buffer_size);
}
...
static Uint8 FC_GrowGlyphCache(FC_Font* font)
{
...
#ifndef FC_USE_SDL_GPU
    {
        ...
        if (prev_target)
        {
            ...
            if (prev_logicalw && prev_logicalh) SDL_RenderSetLogicalSize(font->renderer, prev_logicalw, prev_logicalh);
            else
            {
...
Uint8 FC_UploadGlyphCache(FC_Font* font, int cache_level, SDL_Surface* data_surface)
{
    ...
    GPU_SetAnchor(new_level, 0.5f, 0.5f); // Just in case the default is different
    if (FC_GetFilterMode(font) == FC_FILTER_LINEAR) GPU_SetImageFilter(new_level, GPU_FILTER_LINEAR);
    else GPU_SetImageFilter(new_level, GPU_FILTER_NEAREST);
#else
    SDL_Texture* new_level;
    if (!fc_has_render_target_support) new_level = SDL_CreateTextureFromSurface(font->renderer, data_surface);
    else
    {
        ...
        if (FC_GetFilterMode(font) == FC_FILTER_LINEAR) SDL_SetHint(SDL_HINT_RENDER_SCALE_QUALITY, "1");
...

        {
            ...
            if (prev_target)
            {
                ...
                if (prev_logicalw && prev_logicalh) SDL_RenderSetLogicalSize(renderer, prev_logicalw, prev_logicalh);
                else
                {
...
#else
Uint8 FC_LoadFont(
    FC_Font* font, FC_Target* renderer, const char* filename_ttf, Uint32 pointSize, SDL_Color color, int style)
#endif
{
    SDL_RWops* rwops;
...
#ifdef FC_USE_SDL_GPU
Uint8 FC_LoadFont_RW(
    FC_Font* font, SDL_RWops* file_rwops_ttf, Uint8 own_rwops, Uint32 pointSize, SDL_Color color, int style)
#else
Uint8 FC_LoadFont_RW(FC_Font*   font,
                     FC_Target* renderer,
                     SDL_RWops* file_rwops_ttf,
                     Uint8      own_rwops,
                     Uint32     pointSize,
                     SDL_Color  color,
                     int        style)
...
static FC_Rect FC_RenderLeft(FC_Font* font, FC_Target* dest, float x, float y, FC_Scale scale, const char* text)
{
    ...
    for (; *c != '\0'; c++)
    {
        ...
        dstRect = fc_render_callback(FC_GetGlyphCacheLevel(font, glyph.cache_level), &srcRect, dest, destX, destY,
                                     scale.x, scale.y);
        if (dirtyRect.w == 0 || dirtyRect.h == 0) dirtyRect = dstRect;
        else dirtyRect = FC_RectUnion(dirtyRect, dstRect);
...
static void FC_RenderAlign(
    FC_Font* font, FC_Target* dest, float x, float y, int width, FC_Scale scale, FC_AlignEnum align, const char* text)
{
    switch (align)
    {
    case FC_ALIGN_LEFT: FC_RenderLeft(font, dest, x, y, scale, text); break;
    case FC_ALIGN_CENTER: FC_RenderCenter(font, dest, x + (float)width / 2, y, scale, text); break;
...
static void FC_DrawColumnFromBuffer(
    FC_Font* font, FC_Target* dest, FC_Rect box, int* total_height, FC_Scale scale, FC_AlignEnum align)
{
    int            y = box.y;
    FC_StringList *ls, *iter;
...
FC_Rect FC_DrawBox(FC_Font* font, FC_Target* dest, FC_Rect box, const char* formatted_text, ...)
{
    ...
    if (useClip) set_clip(dest, &oldclip);
    else set_clip(dest, NULL);
...
FC_Rect FC_DrawColumnAlign(
    FC_Font* font, FC_Target* dest, float x, float y, Uint16 width, FC_AlignEnum align, const char* formatted_text, ...)
{
    FC_Rect box = {x, y, width, 0};
    int     total_height;
...
FC_Rect FC_DrawColumnScale(
    FC_Font* font, FC_Target* dest, float x, float y, Uint16 width, FC_Scale scale, const char* formatted_text, ...)
{
    FC_Rect box = {x, y, width, 0};
    int     total_height;
...
FC_Rect FC_DrawColumnColor(
    FC_Font* font, FC_Target* dest, float x, float y, Uint16 width, SDL_Color color, const char* formatted_text, ...)
{
    FC_Rect box = {x, y, width, 0};
    int     total_height;
...
FC_Rect FC_DrawColumnEffect(
    FC_Font* font, FC_Target* dest, float x, float y, Uint16 width, FC_Effect effect, const char* formatted_text, ...)
{
    FC_Rect box = {x, y, width, 0};
    int     total_height;
...
Uint16 FC_GetWidth(FC_Font* font, const char* formatted_text, ...)
{
    ...
    for (c = fc_buffer; *c != '\0'; c++)
    {
        ...
        if (FC_GetGlyphData(font, &glyph, codepoint) || FC_GetGlyphData(font, &glyph, ' ')) width += glyph.rect.w;
    }
    bigWidth = bigWidth >= width ? bigWidth : width;
...
Uint16 FC_GetPositionFromOffset(
    FC_Font* font, float x, float y, int column_width, FC_AlignEnum /* align */, const char* formatted_text, ...)
{
    FC_StringList *ls, *iter;
    Uint8          done      = 0;
...
void FC_SetFilterMode(FC_Font* font, FC_FilterEnum filter)
{
    ...
    if (font->filter != filter)
    {
        ...
        // Update each texture to use this filter mode
        {
            ...
            for (i = 0; i < font->glyph_cache_count; ++i) { GPU_SetImageFilter(font->glyph_cache[i], gpu_filter); }
        }
#endif
    }
}
